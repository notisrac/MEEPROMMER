...
*
**/
#define VERSIONSTRING "MEEPROMMER $Revision: 1.3 $ $Date: 2015/02/28 21:35:54 $, CMD:R,r,w,W,V"
// shiftOut part
#define DS A0
#define LATCH A1
#define CLOCK A2
...
void set_address_bus(unsigned long address)
{
	...
	//get highest byte of the 32 bit address
	byte byteFour = (address >> 24) & 0xFF;
	byte byteThree = (address >> 16) & 0xFF;
	byte byteTwo = (address >> 8) & 0xFF;
	//get lowest byte of the 32 bit address
	byte byteOne = address & 0xff;
	//disable latch line
	bitClear(PORTC, PORTC_LATCH);
	//shift out the bytes
	fastShiftOut(byteFour);
	fastShiftOut(byteThree);
	fastShiftOut(byteTwo);
	fastShiftOut(byteOne);
	//enable latch and set address
	bitSet(PORTC, PORTC_LATCH);
}
...
void fast_write(unsigned long address, byte data)
{
	byte cyclecount = 0;
	//first disbale output
	set_oe(HIGH);
...
byte parseCommand() {
	...
	//first string is the command character
	cmdbuf[1] = 0;
	//second string is startaddress (8 bytes)
	cmdbuf[10] = 0;
	//third string is endaddress (8 bytes)
	cmdbuf[19] = 0;
	//fourth string is length (2 bytes)
	cmdbuf[22] = 0;
	startAddress = hexWord((cmdbuf + 2));
	dataLength = hexWord((cmdbuf + 11));
...
/************************************************************
* convert a hex word (0000 - ffff) to unsigned long
* @param c-string with the hex value of the word
* @return unsigned long represented by the digits
************************************************************/
unsigned long hexWord(char* data) {
	//return ((hexDigit(data[0]) * 4096) +
	//	(hexDigit(data[1]) * 256) +
	//	(hexDigit(data[2]) * 16) +
	//	(hexDigit(data[3])));
	return (
		((unsigned long)hexDigit(data[0]) * 268435456) +
		((unsigned long)hexDigit(data[1]) * 16777216) +
		((unsigned long)hexDigit(data[2]) * 1048576) +
		((unsigned long)hexDigit(data[3]) * 65536) +
		((unsigned long)hexDigit(data[4]) * 4096) +
		((unsigned long)hexDigit(data[5]) * 256) +
		((unsigned long)hexDigit(data[6]) * 16) +
		((unsigned long)hexDigit(data[7]))
		);
}
/************************************************
*
...
**/
void write_block(unsigned long address, byte* buffer, int len) {
